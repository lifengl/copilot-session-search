...
The application starts local-index preparation quietly after the main window loads. The work remains hidden unless AI search needs to wait for it. A literal or regular-expression search pauses preparation by canceling the current resumable pass, preserves every committed session update, and restarts preparation after the search finishes. Turning AI search off does not stop background preparation. Closing the application cancels it safely. The index is stored under `%LOCALAPPDATA%\CopilotSessionSearch`, versioned by schema/parser/chunker/FTS/embedding settings, and incrementally reconciled by session ID and modification time on later launches.
